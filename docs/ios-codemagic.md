...
**Prefer reusing the existing Codemagic Developer Portal key `tradies-toolbox-asc`.** An App Manager ASC API key can sign and upload any app under the same Apple team, including `com.josh12891.roofsetout`. `codemagic.yaml` already points `integrations.app_store_connect` at that name.
...
2. Confirm a key named exactly **`tradies-toolbox-asc`** exists (or add **`au-roof-carpenter-asc`** and update the yaml).
